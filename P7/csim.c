...
#pragma region Handle-Trace
    ...
    while (readline(trace, &op, &address, &request_length) != -1) {
        ...
        int hit = 0;                        // 命中标志，默认未命中
        int empty_line = -1;                // 记录空闲的cache行
        int lru_line = 0;                   // 记录最近最少使用cache行
        int min_lru = set[0].lru_counter;   // 最小的LRU计数器值，即最近最少使用

        // 遍历cache组中的每一块，判断是否命中
        for (int i = 0; i < E; i++) {
            if (set[i].valid) {
                if (set[i].tag == tag)  {
                    // 命中
                    hit = 1;                        //否则，其他情况下hit均为0
                    set[i].lru_counter = lru_time;  //更新LRU计数器
                    hit_count++;
                    break;
                } 
